...
class WorkingParser:
    ...
    def get_all_tasks(self) -> List[Dict[str, Any]]:
        """Получает все задачи из Planfix API"""
        headers = {
            'Authorization': f'Bearer {self.config["rest_api"]["auth_token"]}',
            'Content-Type': 'application/json'
        }
        
        # Запрашиваем основные поля + customFields
        fields = [
            ...
            'closeDate', 'startDate', 'finishDate', 'customFields'
        ]
    ...
    def extract_custom_field_value(self, task: Dict[str, Any], field_name: str) -> Optional[Any]:
        ...
        # Пытаемся найти поле в customFields
        if 'customFields' in task and task['customFields']:
            for field in task['customFields']:
                if field.get('name') == field_name:
                    return field.get('value')
        
        # Если кастомные поля недоступны, возвращаем заглушку
        return None
    ...
    def extract_task_data(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Извлекаем кастомные поля (пока заглушки)
        task_data['client_grade'] = self.extract_custom_field_value(task, 'Грейд клиента') or 'Н/Д'
        task_data['order_percentage'] = self.extract_custom_field_value(task, '% заказа') or 'Н/Д'
        task_data['deal_sum'] = self.extract_custom_field_value(task, 'Сумма сделки') or 'Н/Д'
        task_data['calculation_priority'] = self.extract_custom_field_value(task, 'Расчет - Приоритет просчета') or 'Н/Д'
        task_data['calculation_sum'] = self.extract_custom_field_value(task, 'Сумма просчета') or 'Н/Д'
    ...
    def extract_assignees(self, assignees) -> str:
        ...
        if isinstance(assignees, dict) and 'users' in assignees:
            # Новый формат: {'users': [{'id': 'user:114', 'name': 'Георгий Побединский'}, ...]}
            names = []
    ...
    def calculate_advanced_priority(self, task_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            client_grade = self.parse_numeric(task_data.get('client_grade'), 3.0)
            order_percentage = self.parse_numeric(task_data.get('order_percentage'), 50.0)
    ...
    def update_database(self, tasks_with_data: List[Dict[str, Any]]):
    ...
    def create_analysis_report(self, tasks_with_data: List[Dict[str, Any]]):
        """Создает анализ с доступными данными"""
        try:
            # Создаем DataFrame
            ...
            self.generate_html_report(
                df, status_analysis, priority_analysis, assignee_analysis, assigner_analysis
            )
    ...
    def generate_html_report(self, df: pd.DataFrame, status_analysis: pd.Series, 
                           priority_analysis: pd.Series, assignee_analysis: pd.Series,
...
<head>
    ...
    <title>Анализ Задач Planfix</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            <h1>🚀 Анализ Задач Planfix</h1>
            <p>Анализ с корректной обработкой дат и исполнителей</p>
        </div>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <h3>📅 Задач с датами</h3>
                <div class="stat-value">{len(df[df['end_date'].notna() | df['start_date'].notna()])}</div>
                <p>Задач с указанными датами</p>
            </div>
            
            <div class="stat-card">
                <h3>👥 Задач с исполнителями</h3>
                <div class="stat-value">{len(df[df['assignees'].notna() & (df['assignees'] != '')])}</div>
                <p>Задач с назначенными исполнителями</p>
            </div>
            
            <div class="stat-card">
                <h3>👤 Задач с постановщиками</h3>
                <div class="stat-value">{len(df[df['assigner'].notna() & (df['assigner'] != '')])}</div>
                <p>Задач с указанными постановщиками</p>
            </div>
        </div>
        ...
        <div class="footer">
            ...
            <p>Примечание: Кастомные поля (грейды, суммы) пока недоступны через API</p>
        </div>
    </div>
</body>
</html>
        ...
        with open(f'{self.output_dir}/working_analysis_report.html', 'w', encoding='utf-8') as f:
            f.write(html_content)
        
        print("✅ HTML отчет создан: working_analysis_report.html")
    ...
    def generate_tasks_table_rows(self, df: pd.DataFrame) -> str:
        ...
        for _, task in df.iterrows():
            ...
            date_str = 'Н/Д'
            if task['end_date']:
                date_str = task['end_date'][:10]  # Берем только дату
            elif task['start_date']:
                date_str = task['start_date'][:10]
            ...
            rows += f"""
                <tr>
                    ...
                    <td>{task['assigner'] or 'Н/Д'}</td>
                </tr>
            """
        return rows
    
    def generate_excel_report(self, df: pd.DataFrame, status_analysis: pd.Series, 
                            priority_analysis: pd.Series, assignee_analysis: pd.Series,
                            assigner_analysis: pd.Series):
        ...
        try:
            ...
            self.add_summary_sheet(ws_summary, df, status_analysis, priority_analysis)
            ...
            # Сохраняем файл
            wb.save(f'{self.output_dir}/working_analysis_report.xlsx')
            print("✅ Excel отчет создан: working_analysis_report.xlsx")
            
        except Exception as e:
            print(f"❌ Ошибка создания Excel отчета: {e}")
    
    def add_summary_sheet(self, ws, df: pd.DataFrame, status_analysis: pd.Series, 
                         priority_analysis: pd.Series):
        ...
        data = [
            ...
            ['Задач с датами', len(df[df['end_date'].notna() | df['start_date'].notna()]), 'Задач с указанными датами'],
            ['Задач с исполнителями', len(df[df['assignees'].notna() & (df['assignees'] != '')]), 'Задач с назначенными исполнителями'],
            ['Задач с постановщиками', len(df[df['assigner'].notna() & (df['assigner'] != '')]), 'Задач с указанными постановщиками']
        ]
        ...
        # Анализ по статусам
        ws.cell(row=7, column=1, value='Анализ по статусам').font = Font(bold=True, size=14)
        ws.cell(row=8, column=1, value='Статус')
        ws.cell(row=8, column=2, value='Количество')
        ws.cell(row=8, column=3, value='Процент')
        
        for row, (status, count) in enumerate(status_analysis.items(), 9):
        ...
        # Анализ по приоритетам
        start_row = 9 + len(status_analysis)
    ...
    def add_details_sheet(self, ws, df: pd.DataFrame):
        ...
        # Заголовки
        headers = ['ID', 'Название', 'Статус', 'Приоритет', 'Дата', 'Исполнитель', 'Постановщик']
        for col, header in enumerate(headers, 1):
            cell = ws.cell(row=1, column=col, value=header)
            cell.font = Font(bold=True)
            cell.fill = PatternFill(start_color="366092", end_color="366092", fill_type="solid")
        ...
        for row, (_, task) in enumerate(df.iterrows(), 2):
            ...
            date_str = 'Н/Д'
            if task['end_date']:
                date_str = task['end_date'][:10]
            elif task['start_date']:
                date_str = task['start_date'][:10]
            
            ws.cell(row=row, column=1, value=task['id'])
            ws.cell(row=row, column=2, value=task['name'][:100])
            ...
            ws.cell(row=row, column=6, value=task['assignees'])
            ws.cell(row=row, column=7, value=task['assigner'])
    ...
    def run_full_analysis(self):
        ...
        self.update_database(tasks_with_data)
        
        # Создаем анализ
        print("📊 Создание анализа...")
        self.create_analysis_report(tasks_with_data)
        
        print("✅ Рабочий анализ завершен!")
