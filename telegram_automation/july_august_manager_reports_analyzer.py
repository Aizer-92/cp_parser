#!/usr/bin/env python3
"""
July-August Manager Reports Analyzer for Main mega ТОП
Полные отчеты по каждому руководителю за июль и август 2025
"""
...
class JulyAugustManagerReportsAnalyzer:
    def __init__(self):
        ...
        logger.add("output/logs/july_august_reports.log", rotation="1 day", retention="7 days")
    ...
    async def get_july_august_messages(self):
        """Получение сообщений за июль и август 2025"""
        try:
            print("📥 Загрузка сообщений за июль и август 2025...")
            
            # Определяем период июль-август 2025
            july_start = datetime(2025, 7, 1)
            august_end = datetime(2025, 8, 31, 23, 59, 59)
            
            # Загружаем сообщения с начала июля
            ...
            print(f"📊 Отфильтровано {len(filtered_messages)} сообщений за июль-август")
            
            # Если нет данных, попробуем загрузить больше сообщений
            if len(filtered_messages) == 0:
                print("🔄 Попытка загрузки большего количества сообщений...")
                messages = await self.client.get_messages(
                    self.target_chat_id,
                    limit=50000
                )
                ...
                print(f"📊 После расширенной загрузки: {len(filtered_messages)} сообщений за июль-август")
    ...
    def extract_plan_fact_sections(self, text, sender, message_date):
        ...
        try:
            ...
            # Если не найдены разделы с ключевыми словами, пробуем альтернативные методы
            if not plan_section and not fact_section:
                # Метод 1: Ищем числовые списки (1., 2., 3. и т.д.)
                numbered_lines = []
                for line in lines:
                    line = line.strip()
                ...
                # Метод 2: Ищем строки с дефисами или маркерами
                if not plan_section and not fact_section:
                    bullet_lines = []
                    for line in lines:
                        line = line.strip()
                ...
                # Метод 3: Простое разделение по количеству строк
                if not plan_section and not fact_section:
    ...
    def analyze_july_august_data(self, weekly_messages):
        """Анализ данных за июль-август по руководителям"""
        try:
            print("🔍 Анализ данных за июль-август по руководителям...")
                ...
                for sender in set(current_week_plans.keys()) | set(next_week_facts.keys()):
                    ...
                    if plan_tasks or fact_tasks:
                        ...
                        if comparison:
                            ...
                            manager_data[sender]['all_unmatched_plans'].extend(comparison['unmatched_plans'])
                            manager_data[sender]['all_new_tasks'].extend(comparison['unmatched_facts'])
    ...
    def create_july_august_reports(self, manager_data, filename=None):
        """Создание полных отчетов за июль-август"""
        try:
            if not filename:
                filename = f"output/july_august_manager_reports_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            ...
            for sender, data in manager_data.items():
                if data['weeks']:  # Только если есть данные
                    self.create_full_manager_sheet(workbook, sender, data)
            
            workbook.save(filename)
            print(f"✅ Полные отчеты за июль-август созданы: {filename}")
            return filename
    ...
    def create_summary_sheet(self, workbook, manager_data):
        ...
        try:
            ...
            # Заголовок
            ws['A1'] = "ПОЛНЫЕ ОТЧЕТЫ ПО РУКОВОДИТЕЛЯМ - ИЮЛЬ-АВГУСТ 2025"
            ws['A2'] = "Main mega ТОП - Детальный анализ"
            ...
            ws['A4'] = "📊 ОБЩАЯ СТАТИСТИКА ЗА ИЮЛЬ-АВГУСТ:"
    ...
    def create_full_manager_sheet(self, workbook, sender, data):
        """Создание полного листа для конкретного руководителя"""
        try:
            ...
            ws['A1'] = f"ПОЛНЫЙ ОТЧЕТ ПО РУКОВОДИТЕЛЮ: {sender}"
            ws['A2'] = f"Период: Июль-Август 2025"
            
            # Общая статистика руководителя
            completion_rate = (data['total_matched'] / data['total_planned'] * 100) if data['total_planned'] > 0 else 0
            status = self.get_overall_status(completion_rate, data['total_matched'], data['total_planned'])
            
            ws['A4'] = "📊 ОБЩАЯ СТАТИСТИКА РУКОВОДИТЕЛЯ:"
            ...
            # Детализация по неделям (в хронологическом порядке)
            ws['A14'] = "📅 ДЕТАЛИЗАЦИЯ ПО НЕДЕЛЯМ (ХРОНОЛОГИЧЕСКИ):"
            ...
            sorted_weeks = sorted(data['weeks'], key=lambda x: x['plan_week'])
            ...
            # ПОЛНЫЙ СПИСОК СОПОСТАВЛЕННЫХ ЗАДАЧ
            if data['all_matches']:
                ws['A' + str(row + 2)] = "✅ ПОЛНЫЙ СПИСОК СОПОСТАВЛЕННЫХ ЗАДАЧ:"
            ...
            # ПОЛНЫЙ СПИСОК НЕВЫПОЛНЕННЫХ ЗАДАЧ
            if data['all_unmatched_plans']:
                start_row = match_row + 2 if data['all_matches'] else row + 2
                ws['A' + str(start_row)] = "❌ ПОЛНЫЙ СПИСОК НЕВЫПОЛНЕННЫХ ЗАДАЧ:"
                
                unmatched_headers = ['Неделя плана', 'Задача из плана', 'Примечание']
                for col, header in enumerate(unmatched_headers, 1):
                    ws.cell(row=start_row + 1, column=col, value=header)
                
                unmatched_row = start_row + 2
                for plan_task in data['all_unmatched_plans']:
                    # Находим неделю для этой задачи
                    week_info = self.find_week_for_plan_task(plan_task, data['weeks'])
                    
                    ws.cell(row=unmatched_row, column=1, value=week_info['plan_week'])
                    ws.cell(row=unmatched_row, column=2, value=plan_task)
                    ws.cell(row=unmatched_row, column=3, value="Задача из плана не найдена в фактах")
                    
                    # Форматирование ячеек с текстом
                    cell = ws.cell(row=unmatched_row, column=2)
                    cell.alignment = Alignment(wrap_text=True, vertical='top')
                    ws.row_dimensions[unmatched_row].height = 60
                    
                    unmatched_row += 1
            
            # ПОЛНЫЙ СПИСОК НОВЫХ ЗАДАЧ
            if data['all_new_tasks']:
                start_row = unmatched_row + 2 if data['all_unmatched_plans'] else (match_row + 2 if data['all_matches'] else row + 2)
                ws['A' + str(start_row)] = "🆕 ПОЛНЫЙ СПИСОК НОВЫХ ЗАДАЧ:"
                
                new_headers = ['Неделя факта', 'Задача из факта', 'Примечание']
                for col, header in enumerate(new_headers, 1):
                    ws.cell(row=start_row + 1, column=col, value=header)
                
                new_row = start_row + 2
                for fact_task in data['all_new_tasks']:
                    # Находим неделю для этой задачи
                    week_info = self.find_week_for_fact_task(fact_task, data['weeks'])
                    
                    ws.cell(row=new_row, column=1, value=week_info['fact_week'])
                    ws.cell(row=new_row, column=2, value=fact_task)
                    ws.cell(row=new_row, column=3, value="Задача из фактов не была в планах")
            ...
            # Форматирование
            self.format_full_manager_sheet(ws)
            
        except Exception as e:
            logger.error(f"❌ Ошибка создания полного листа руководителя {sender}: {e}")
    ...
    def find_week_for_plan_task(self, plan_task, weeks):
        """Найти неделю для задачи из плана"""
        for week in weeks:
            if plan_task in week['plan_tasks']:
                return {'plan_week': week['plan_week']}
        return {'plan_week': 'N/A'}
    
    def find_week_for_fact_task(self, fact_task, weeks):
        """Найти неделю для задачи из факта"""
        for week in weeks:
            if fact_task in week['fact_tasks']:
                return {'fact_week': week['fact_week']}
        return {'fact_week': 'N/A'}
    ...
    def format_full_manager_sheet(self, ws):
        """Форматирование полного листа руководителя"""
        # Заголовок
    ...
    async def run_july_august_reports(self):
        """Запуск создания полных отчетов за июль-август"""
        try:
            print("🚀 ЗАПУСК СОЗДАНИЯ ПОЛНЫХ ОТЧЕТОВ ЗА ИЮЛЬ-АВГУСТ")
            print("=" * 70)
            ...
            # Получение сообщений за июль-август
            weekly_messages = await self.get_july_august_messages()
            
            if not weekly_messages:
                print("❌ Не найдено сообщений за июль-август")
                return
            
            # Анализ данных по руководителям
            manager_data = self.analyze_july_august_data(weekly_messages)
            
            if not manager_data:
                print("❌ Не найдено данных по руководителям за июль-август")
                return
            
            # Создание отчетов
            report_file = self.create_july_august_reports(manager_data)
            
            if report_file:
                print(f"\n🎉 Полные отчеты за июль-август созданы!")
                print(f"📁 Файл сохранен: {report_file}")
                
                # Выводим краткую сводку
                print(f"\n📊 КРАТКАЯ СВОДКА ЗА ИЮЛЬ-АВГУСТ:")
                ...
                print(f"\n👥 РУКОВОДИТЕЛИ С ДАННЫМИ ЗА ИЮЛЬ-АВГУСТ:")
                for sender, data in sorted(manager_data.items(), key=lambda x: x[1]['total_matched'], reverse=True):
                    completion_rate = (data['total_matched'] / data['total_planned'] * 100) if data['total_planned'] > 0 else 0
                    weeks_count = len(data['weeks'])
...
async def main():
    analyzer = JulyAugustManagerReportsAnalyzer()
    await analyzer.run_july_august_reports()
