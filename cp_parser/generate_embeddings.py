...
С graceful error handling - если ошибка, пропускаем товар
"""
...
try:
    ...
    print("⚠️  OpenAI не доступен. Установите: pip install openai")
...
def generate_all_embeddings(batch_size=10, limit=None):
    ...
    if not OPENAI_AVAILABLE:
        print("❌ OpenAI недоступен. Установите: pip install openai")
    ...
    with db_manager.get_session() as session:
        try:
            ...
            if not check:
                print("❌ Колонка name_embedding не существует")
                print("   Запустите сначала: python3 setup_vector_search.py")
                return
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'test':
        ...
        if count is not None:
            ...
            print("⚠️  Не удалось проверить статус. Запустите setup_vector_search.py сначала")
