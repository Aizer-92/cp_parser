...
def api_search_by_image():
    ...
    print(f"📸 [IMAGE SEARCH] Получен запрос на поиск по изображению")
    
    # Проверяем доступность pgvector БД
    if not PGVECTOR_ENGINE:
        print(f"⚠️  [IMAGE SEARCH] PGVECTOR_ENGINE не настроен")
        return jsonify({
            'success': False, 
            'error': 'Поиск по изображениям временно недоступен. Требуется настройка базы данных векторов (VECTOR_DATABASE_URL).'
        }), 503
