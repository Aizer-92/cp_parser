#!/usr/bin/env python3
"""
Загрузка правильных данных: original_sheet + Вторая таблица + Мерч для Sense
"""
...
def load_products():
    ...
    # Загружаем товары из оригинальной таблицы
    with open('parsed_products.json', 'r', encoding='utf-8') as f:
        all_products = json.load(f)
    
    # Фильтруем нужные таблицы
    ...
    # Загружаем товары из Мерч для Sense (с правильными ценами)
    ...
    print(f"  - Оригинальная таблица: {len(original_products)} товаров")
...
def add_images():
    ...
    with db.get_session() as session:
        ...
        for product in products:
            ...
            # Для ежедневников (Вторая таблица)
            elif product.sheet_id == 2:  # Вторая таблица
                # Ищем изображения в папке storage/images/products_Вторая таблица
                image_dir = "storage/images/products_Вторая таблица_1757933430"
                if os.path.exists(image_dir):
                    for filename in os.listdir(image_dir):
                        if filename.endswith('.jpg'):
                            full_path = os.path.abspath(os.path.join(image_dir, filename))
            ...
            elif product.sheet_id == 3:  # Мерч для Sense
...
def main():
    """Основная функция"""
    print("🚀 Загрузка правильных данных")
    print("=" * 50)
